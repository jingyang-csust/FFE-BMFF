...
# ---------------------------------- #
from lib.models.layers.lama_fft1_plus9 import SG_Block
# ---------------------------------- #
...
class CEABlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, stages, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.adap_fft_x = SG_Block()
        self.adap_fft_xi = SG_Block()
    ...
    def forward(self, x, xi, global_index_template, global_index_templatei, global_index_search, global_index_searchi,
        ...
        # ---------------lama1------------------- #
        if self.stage < 9:
            x = x + self.adap_fft_x(self.norm3(x))
            xi = xi + self.adap_fft_xi(self.norm4(xi))
        # ---------------------------------- #

        # ---------------lama1_plus------------------- #
        # x = self.adap_fft_x(self.norm3(x))
        # xi = self.adap_fft_xi(self.norm4(xi))
        # ---------------------------------- #
